...
folder are mapped to the application's root namespace, 'ChebAfisha'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "ChebAfisha"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'ChebAfisha' in the same way
as `"ChebAfisha/sass/src"`.
